...
if (!API_BASE_URL) {
    console.warn('Warning: NEXT_PUBLIC_API_URL is not set. API requests may fail.');
}
...
async function fetchApi<T>(
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return {
                error: (errorBody && typeof errorBody === 'object' && 'message' in errorBody) ? (errorBody as { message?: string }).message : `Error: ${response.status}`,
