...
export const useCreateShortUrl = (options?: UseMutationOptions<ShortUrl, Error, CreateShortUrlInput>) => {
...
export const useGetUserUrls = (options?: UseQueryOptions<ShortUrl[], Error>) => {
...
export const useDeleteUrl = (options?: UseMutationOptions<void, Error, string>) => {
    return useMutation<void, Error, string>({
        ...
        onSuccess: (data, variables, context) => {
            queryClient.invalidateQueries({ queryKey: urlsQueryKeys.lists() });
            const savedUrls = JSON.parse(localStorage.getItem('shortenedUrls') || '[]');
            const updatedUrls = savedUrls.filter((url: ShortUrl) => url.id !== variables);
