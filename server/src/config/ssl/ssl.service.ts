...
export class SSLService {
     ...
     */
    static async createSelfSignedCertificate(options: CertificateOptions = {}): Promise<SelfSignedCertificate | undefined> {
        const {
            host = 'localhost',
        ...
        try {
            ...
            const existingCert = await this.validateExistingCertificate(keyPath, certPath, host, binaryPath);
            if (existingCert) {
                return existingCert;
            }

            // Generate new certificates
            return await this.generateNewCertificate(binaryPath, keyPath, certPath, host, hosts, certDir);
        } catch (err) {
            this.logger.error('Failed to generate self-signed certificate', err);
            this.logger.error('💡 If you see permission errors, try running as administrator/sudo');
    ...
    private static async validateExistingCertificate(
        ...
        try {
            ...
            this.logger.warn('⚠️ Existing certificate validation failed, generating new certificate');
    ...
    private static async generateNewCertificate(
        ...
        this.logger.log('🔐 Generating self-signed certificate with automatic CA installation...');
        this.logger.log('📋 This process will install a root CA certificate in your system trust store');

        const hostsToUse = host && !defaultHosts.includes(host)
            ? [...defaultHosts, host]
            : defaultHosts;

        this.logger.debug(`🌐 Certificate will be valid for: ${hostsToUse.join(', ')}`);
        ...
        try {
            ...
            this.logger.error('💡 You may need to run with administrator/sudo privileges');
            this.logger.error('💡 On Windows: Right-click terminal and "Run as administrator"');
        ...
        this.logger.log(`📄 Certificate files created in: ${resolve(process.cwd(), certDir)}`);
    ...
    private static async addToGitignore(certDir: string): Promise<void> {
        try {
            ...
            if (existsSync(gitignorePath)) {
                ...
                this.logger.debug('📝 No .gitignore file found, skipping certificate directory exclusion');
            }
        } catch (err) {
     ...
     */
    static async createHttpsOptions(): Promise<Record<string, unknown> | undefined> {
        if (process.env.NODE_ENV !== 'development') {
            this.logger.debug('⏭️ Skipping SSL certificate generation (not in development mode)');
            return undefined;
        }

        this.logger.log('🚀 Initializing SSL certificate setup for development server');

        try {
            const cert = await this.createSelfSignedCertificate();

            if (!cert) {
                this.logger.warn('⚠️ SSL certificate generation failed, starting server in HTTP mode');
                return undefined;
            }

            this.logger.log('✅ SSL certificates ready - HTTPS server will be available');
